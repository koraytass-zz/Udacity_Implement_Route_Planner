...
def create_closedSet(self):
    ...
    suitableDataStructure = set()
    return suitableDataStructure
...
def create_openSet(self):
    ...
    if self.start != None:
        ...
        suitableDataStructure = set() 
        suitableDataStructure.add(self.start)
        return (suitableDataStructure)
...
def get_current_node(self):
    ...
    for i in self.openSet:
        suitableDataStructure[i] = self.calculate_fScore(i)
        suitableValue = min(suitableDataStructure.keys(), key=(lambda k: suitableDataStructure[k]))
...
# 
# **ANSWER**:
# I think A* algorithm minimum time and guarantees finding the optimal path. Reducing the cost in exploring path affect time performance.And, its directed towars to goal.
# 
# 
...
# 
# **ANSWER**:
# A* algorithm guarantees to optimal, cheaper and fastest way. On the other hand, Uniform cost serach algorithm can go away from the optimal solution. 
# 
# A* algorithm is uninformed algorithm but Best First search algoritm is informed. Best First selects lowest heuristics value for reaching the goal node. A star algorithm takes the cost of reaching intermediate + heuristic value to reach goal node into account.
# 
# According to review :
# Best First using  heuristics function f(n) = h with the lowest heuristic value.
# On the other hand, A* algorithm heuristics f(n) = h + g 

# ---
# What is a heuristic?
# 
# **ANSWER**:Heuristic is an approach which enables us to solve any problem with the help of partial data ( or some data about the environment) in order to learn or discover more about the problem or environment.

# ---
# What is a consistent heuristic?
# According to review:
# **ANSWER**:Consistent heuristic estimate is always less than or equal distance from node to neighbor + h(neighbor).

# ---
# What is a admissible heuristic? 
# 
# **ANSWER**:if it never overestimates the cost of reaching the goal, i.e. the cost it estimates to reach the goal is not higher than the lowest possible cost from the current point in the path.
# 
...
#     - None
#     
# **ANSWER**:Somedmissible heuristic are consistent.
